...
resultsfile=$workingdir/resultsfile
if [ -f $resultsfile ]; then
  mv $resultsfile $resultsfile.`date +%Y%m%d%H%M`
fi
...
do
  ...
  do
    ...
    echo "**********************************************************************" >> $resultsfile
    echo "$mod" >> $resultsfile

    cd ./testresults
    grep -ri failures >>  $resultsfile
    echo "**********************************************************************" >> $resultsfile
  done
  cd $workingdir
done
